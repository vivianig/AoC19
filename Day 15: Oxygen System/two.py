...
def calculate_oxygen_spread(oxygen_pos):
    filled = [oxygen_pos]
    stack = [oxygen_pos]
    time = -1
    while stack:
        ...
        for x in range(len(stack)):
            ...
            filled.append(pos)
            grid[pos] = -1
            for new_pos in [(pos[0] + directions[z][0], pos[1] + directions[z][1]) for z in directions]:
                if new_pos not in grid or grid[new_pos] != 1 or new_pos in filled:
                    continue
...
print_map()
calculate_oxygen_spread([x for x in grid if grid[x] ==2][0])
